...
public class BinarySearch {
     ...
     * @return
     */
    public static int binarySearch1ByMyself(int[] array, int low, int high, int target) {
        while (low <= high) {
            // int mid = (low + high) >> 1;
            // 移位运算的优先级比加减低
...
//1.右移提高性能
     ...
     * @return
     */
    public static int binarySearch2ByMyself(int[] array, int low, int high, int target) {
        if (low < high) {
            int mid = low + ((high - low) >> 1);
            if (array[mid] == target) {
                return mid;
            } if (array[mid] < target) {
                return binarySearch2ByMyself(array, mid + 1, high, target);
            } if (array[mid] > target) {
                return binarySearch2ByMyself(array, low, mid - 1, target);
            }
        }
        return -1;
    }
