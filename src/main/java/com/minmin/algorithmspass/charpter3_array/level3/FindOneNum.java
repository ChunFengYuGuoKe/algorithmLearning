...
public class FindOneNum {
    public static void main(String[] args) {
        int[] arr = {4, 1, 2, 1, 2};
        System.out.println(findOneNumByMySelf(arr));
        System.out.println(findOneNum2ByMySelf(arr));
    }

    public static Integer findOneNumByMySelf(int[] arr) {
        Set<Integer> set = new HashSet<Integer>();
        for (int i = 0; i < arr.length; i++) {
            if(!set.add(arr[i])) set.remove(arr[i]);
        }
        if(set.size() == 0) return null;
        return set.toArray(new Integer[set.size()])[0];
    }

    public static int findOneNum2ByMySelf(int[] arr) {
        int result = 0;
        for (int i : arr) {
            result ^= i;
        }
